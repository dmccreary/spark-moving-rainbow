...
declare variable $util2:nl :="&#10;";
declare variable $util2:context := request:get-context-path(); (: / or /exist :)
declare variable $util2:conditional-port :=
  if (request:get-server-port() = 80)
    then ()
    else concat(':', string(request:get-server-port()));
    
declare variable $util2:url-base :=
  concat(
    'http://',
    request:get-server-name(),
    $util2:conditional-port,
    $util2:context
    );
...
declare function util2:list-files($base as xs:string) as node() {
let $collection := $base
...
(: converts date time into "Jan 12, 2014 10:23am" format :)
...
(: just the first five digits of the time - hours and minutes.  no seconds. :)
...
(: XML to XHTML for display in browser dispatcher 
   See http://www.w3.org/TR/xquery/#dt-node-test
:)
...
for $node in $input
   return 
      typeswitch($node)
        ...
        case document-node()
             return <div class="document-node">{util2:xml-to-html($node/node(), $depth + 1)}</div>
        case comment()
             return <div class="comment">{$node/comment()}</div>
        case element()
           return
              <div class="element" style="{$left-margin}">
                    ...
                    { (: for each attribute create two spans for the name and value :)
                     for $att in $node/@*
                        return
                           (
                              <span class="an">{' '} {name($att)}=</span>,<span class="av">"{string($att)}"</span>
                           )
                    }
...
declare variable $util2:xml-to-html-colors-css-link :=
<link type="text/css" href="../resources/css/xml-to-html-colors.css" rel="stylesheet"/>
;

declare variable $util2:xml-to-html-colors-css-link-orbeon :=
<link type="text/css" href="../resources/css/xml-to-html-colors-orbeon.css" rel="stylesheet"/>
;

(: note that all strings are wrapped in double quotes in case they contain commas 
   There is a " before the first string, the seperator is ", " and there is a final double quote.
   :)
declare function util2:table-to-csv ($table as node()) as xs:string {
let $quote := "&#34;"
let $field-seperator := concat($quote, ', ', $quote)
let $csv-string := string-join(
        for $row in $table//tr
        return
          concat($quote,
               string-join(
...
(: note that all strings are wrapped in double quotes in case they contain commas 
   There is a " before the first string, the seperator is ", " and there is a final double quote.
   Note that Excel has problems with unicode double quotes as Text Qualifiers
   http://office.microsoft.com/en-us/excel-help/text-import-wizard-HP010102244.aspx?CTT=5&origin=HP010099725
   :)
declare function util2:table-to-csv ($table as node(), $file-name as xs:string) as xs:string {
let $quote := "&#34;"
let $field-seperator := concat($quote, ', ', $quote)
let $csv-string := string-join(
        for $row in $table//tr
...
let $attachment-string := concat('attachment; filename="', $file-name, '"')
let $set-file-name := response:set-header('Content-Disposition',   $attachment-string)
...
declare function util2:table-to-tab-delimited($table as node(), $file-name as xs:string) as xs:string {
let $tab := '&#9;' (: unicode for tab :)
let $csv-string := string-join(
        for $row in $table//tr
        return
           string-join(
