...
let $spark-response-code := s:new-pattern($new-pattern)

let $log := 
   if ($debug)
      then util:log-system-out(concat('new-pattern.xq?new-pattern=', $new-pattern))
      else ()

return
<led-state-change>
  <new-pattern>{$new-pattern}</new-pattern>
  <spark-response-code>{$spark-response-code}</spark-response-code>
